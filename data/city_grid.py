...
def create_city_grid(size=20, num_stations=6):
    ...
    mean_sea_level = 0
    elevation_base = np.random.uniform(10, 100)  # land elevation above MSL
    stream_elevation_base = np.random.uniform(5, 9)  # stream slightly above MSL but lower than land
    ...
    for i in range(size):
        for j in range(size):
            ...
            if (i, j) in stream_cells:
                elevation = stream_elevation_base + np.random.uniform(-1, 1)
            else:
                elevation = elevation_base + np.random.uniform(-5, 5)
            zone = {
                "x": i,
                "y": j,
    ...
    stream_cells = list(stream_cells)
    stream_cells.sort()

    for idx, (x, y) in enumerate(stream_cells):
        for zone in zones:
            if zone["x"] == x and zone["y"] == y:
                zone["elevation"] = stream_elevation_base - idx * 0.1  # downhill slope
                break
